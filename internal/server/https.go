...
func (h *HTTPS) saveCertToFile() error {
	...
	if err := pem.Encode(&certPEM, &pem.Block{
		...
		logrus.WithError(err).Errorf("filed to create %s file", CertPEM)
		return err
	}
	err := os.WriteFile(CertPEM, certPEM.Bytes(), 0644)
	if err != nil {
		logrus.Error(err)
		return err
	}
...
func (h *HTTPS) saveKeyToFile() error {
	...
	if err := pem.Encode(&privateKeyPEM, &pem.Block{
		...
		logrus.WithError(err).Errorf("filed to create %s file", PrivateKeyPEM)
		return err
	}
	err := os.WriteFile(PrivateKeyPEM, privateKeyPEM.Bytes(), 0644)
	if err != nil {
		logrus.Error(err)
		return err
	}
